...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for s in sents:
            sents_char_indices = []
            for w in s:
                sents_char_indices.append([self.start_of_word]+[self.char2id[c] for c in w]+[self.end_of_word])
            char_indices.append(sents_char_indices)
        return char_indices
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        # (batch_size, max_sent, max_word)
        word_char_ids = self.words2charindices(sents)
        padded_chars = pad_sents_char(word_char_ids, self.char_unk)
        sents_var = torch.tensor(padded_chars, dtype=torch.long, device=device)
        sents_var = torch.transpose(sents_var, 0, 1)  # (max_sent, batch_size, max_word)
        return sents_var
        ### END YOUR CODE
